...
const ModalExample = ({ toggle }) => {
  ...
  const sendPhoneNumberHandler = () => {
    ...
    if (isValidNumber(login)) {
      ...
      fetch('http://localhost:4000/user/reg', options)
        .then(async res => {
  ...
  const verifyCode = () => {
    ...
    if (isValidCode(code)) {
      ...
      fetch('http://localhost:4000/user/code', options)
        .then(async res => {
          if (res.status === 200) {
  ...
  const isSpaceAdd = (prevValue, newValue) => {
    if (prevValue.length > newValue.length || prevValue.length === newValue.length) {
      return false;
    }
    ...
    <div className={style.modalRegistration}>
      ...
      <div onSubmit={
        ...
        <form action="">
          ...
          />
          {/* /!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  переделать */}
          <button onClick={() => sendPhoneNumberHandler()/*setShowSms(!showSms)*/} type="button" className="btn btn-primary w-40 " >Продолжить</button>
        </form>
      </div>
      <ModalFooter >
  ...
  (<div className={style.modalRegistration}>
    ...
    <div className={style.body}>
      <form action="">
        <input type="text" required placeholder='Имя' value={name} onChange={(e) => { if (!isSpaceAdd(name, e.target.value)) setName(e.target.value) }} />
        <input type="password" required placeholder='Пароль' value={password} onChange={(e) => { if (!isSpaceAdd(password, e.target.value)) setPassword(e.target.value) }} />

        {/* /!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  переделать */}
        <button type="button" className="btn btn-primary w-40 " onClick={() => endRegistrationHandler()} >Зарегистрироваться </button>
      </form>
    </div>
  </div>);
  ...
  (
    <div className={style.modalRegistration}>
      ...
      <div className={style.body}>
        <form action="">
          <input value={code} onChange={(e) => setCode(e.target.value)} autoFocus type="text" className='mask-phone' required placeholder='Код подтверждения' />
          <span className={style.link}>Получить новый код</span>
          <div className="d-flex mt-2">
            <button onClick={() => { verifyCode() }} type="button" className="btn btn-primary w-40 " >Подтвердить</button>
            <button onClick={() => changeNumberHandler()} type="button" className="btn w-40  btn-secondary">Изменить номер </button>
          </div>
        </form>
      </div>
    </div>
  ...
  <div className={style.modal}>
    ...
    <div className={style.body}>
      <form action="">
        <input type="text" className='mask-phone' value={login} onChange={(e) => { if (!isSpaceAdd(login, e.target.value)) setLogin(e.target.value) }} required placeholder='Телефон или электронная почта' />
        <input type="password" onKeyPress={(e) => e.key === 'Enter' ? loginHandler() : null} required value={password} onChange={(e) => { if (!isSpaceAdd(password, e.target.value)) setPassword(e.target.value) }} placeholder='Пароль' />
        ...
        <button type="button" class="btn btn-primary w-50 " onClick={loginHandler}>Войти</button>
      </form>
    </div>
    <ModalFooter >
      <div className={style.footer}>
        <p onClick={() => { setLogin(''); setPassword(''); setShowSms(true); toggleReg() }}>Зарегистрироваться</p>
      </div>
    </ModalFooter>
  </div>)
